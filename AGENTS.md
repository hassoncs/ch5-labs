...
CH5's monorepo for small, weird experiments. `apps/*` contains private browser toys.
`packages/*` contains reusable browser-safe libraries and the public npm template.
...
- **Browser-safe product code only.** No node builtins or top-level `process` in
  `apps/*/src` or `packages/*/src`. The repo intentionally has no `@types/node`.
...
```
apps/*/              private browser toys
packages/template/   copy to start a public npm experiment
packages/*/          shared browser-safe libraries
